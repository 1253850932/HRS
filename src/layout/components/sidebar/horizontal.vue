...
const { route, title, routers, logout, backHome, onPanel, menuSelect, username, avatarsStyle } = useNav()
...
<template>
  <div v-loading="usePermissionStoreHook().wholeMenus.length === 0" class="horizontal-header">
    ...
    <el-menu router ref="menuRef" mode="horizontal" class="horizontal-header-menu" :default-active="route.path" @select="indexPath => menuSelect(indexPath, routers)">
